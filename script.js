window.onload = function() {
    ...
    const soundManager = {
        ...
        loadSounds: async function(soundList) {
            if (!audioCtx) return;
            const promises = Object.entries(soundList).map(async ([key, url]) => {
                try {
        ...
        playSound: function(key) {
            if (this.isMuted || !this.sounds[key] || !audioCtx || audioCtx.state === 'suspended') return;
        ...
        playMusic: function(key) {
            if (!this.sounds[key] || this.isMusicPlaying || !audioCtx) return;
            this.stopMusic();
            this.musicSource = audioCtx.createBufferSource();
            this.musicSource.buffer = this.sounds[key];
            this.musicSource.loop = true;
            this.musicSource.connect(audioCtx.destination);
        ...
        startLoop: function(key) {
            if (this.isMuted || !this.sounds[key] || this.loopingSounds[key] || !audioCtx || audioCtx.state === 'suspended') return;
            const source = audioCtx.createBufferSource();
            source.buffer = this.sounds[key];
            source.loop = true;
            source.connect(audioCtx.destination);
            source.start(0);
            this.loopingSounds[key] = source;
        },

        stopLoop: function(key) {
            if (this.loopingSounds[key]) {
                this.loopingSounds[key].stop();
                delete this.loopingSounds[key];
            }
        },
        
        muteFX: function(isMuted) {
    ...
    function handleTouches(e) { e.preventDefault(); if (e.type === 'touchstart') { for (let i = 0; i < e.changedTouches.length; i++) { const touch = e.changedTouches[i]; const distJump = Math.sqrt((touch.clientX - jumpButton.x)**2 + (touch.clientY - jumpButton.y)**2); if (distJump < jumpButton.radius) handleJumpInput(); const distReset = Math.sqrt((touch.clientX - resetButton.x)**2 + (touch.clientY - resetButton.y)**2); if (distReset < resetButton.radius) resetGame(); } } isTouchingLeft = false; isTouchingRight = false; for (let i = 0; i < e.touches.length; i++) { const touch = e.touches[i]; const distLeft = Math.sqrt((touch.clientX - leftButton.x)**2 + (touch.clientY - leftButton.y)**2); if (distLeft < leftButton.radius) isTouchingLeft = true; const distRight = Math.sqrt((touch.clientX - rightButton.x)**2 + (touch.clientY - rightButton.y)**2); if (distRight < rightButton.radius) isTouchingRight = true; } }
    ...
    function updatePlayer(time) {
        ...
        let onRainbowPlatform = player.onGround && player.standingOnPlatform && player.standingOnPlatform.type === 'rainbow';
        ...
        if (jumpBufferValid && player.onGround) { player.dy = JUMP_FORCE; player.onGround = false; jumpBufferTime = 0; if (onRainbowPlatform) { soundManager.playSound('super_jump'); } else { soundManager.playSound('jump'); } }
        ...
        player.worldY += player.dy; player.onGround = false; player.standingOnPlatform = null;
        for (const p of physicalObjects) { if (checkPlatformCollision(player, p)) { if (player.dy >= 0 && lastPlayerY + player.radius <= p.worldY + 1) { player.worldY = p.worldY - player.radius; player.dy = 0; player.onGround = true; player.standingOnPlatform = p; } else if (player.dy < 0) { player.worldY = p.worldY + p.height + player.radius; player.dy = 0; } } }
        for (const coin of iceCoins) { if (coin.active) { const distSq = (player.worldX - coin.worldX)**2 + (player.worldY - coin.worldY)**2; if (distSq < (player.radius + coin.radius)**2) { coin.active = false; player.isFrozen = true; player.freezeEndTime = time + 3000; player.dx = 0; player.dy = 0; soundManager.playSound('freeze'); } } }
            ...
            if (coin.active) {
                ...
                if (distSq < (player.radius + coin.radius)**2) {
                    ...
                    const newAttackEvent = { 
                        count: 10,
                        nextSpawnTime: time + 2000,
                        loopSound: 'danger_loop'
                    };
                    setTimeout(() => {
                        if (!player.isDead && attackEvents.includes(newAttackEvent)) {
                           soundManager.startLoop(newAttackEvent.loopSound);
                        }
                    }, 2000);
                    attackEvents.push(newAttackEvent);
                }
            }
        }
        ...
        player.rotationAngle += player.dx * 0.02; if (player.worldX > highestX) highestX = player.worldX; if (player.worldY > viewHeight / 2 + height + 800) { if (!gameCleared) { player.isDead = true; soundManager.muteFX(true); soundManager.playSound('hit'); attackEvents.forEach(e => soundManager.stopLoop(e.loopSound)); attackEvents = []; setTimeout(() => init(currentStage, false), 100); } }
    }
    ...
    function updateAndDrawClearEffects() { if (!fireworksLaunched) { launchFireworks(); fireworksLaunched = true; } for (let i = rockets.length - 1; i >= 0; i--) { const r = rockets[i]; r.x += r.dx; r.y += r.dy; r.dy += 0.2; ctx.fillStyle = `hsl(${r.hue}, 100%, 75%)`; ctx.beginPath(); ctx.arc(r.x, r.y, 3, 0, Math.PI * 2); ctx.fill(); if (r.y <= r.targetY) { createExplosion(r.x, r.y, r.hue); soundManager.playSound('hit'); rockets.splice(i, 1); } } let lastCompositeOperation = ctx.globalCompositeOperation; ctx.globalCompositeOperation = 'lighter'; for (let i = particles.length - 1; i >= 0; i--) { const p = particles[i]; p.x += p.dx; p.y += p.dy; p.dy += GRAVITY * 0.08; p.dx *= 0.98; p.life--; if (p.life <= 0) { particles.splice(i, 1); continue; } ctx.fillStyle = `hsla(${p.hue}, 100%, 70%, ${p.life / 90})`; ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2); ctx.fill(); } ctx.globalCompositeOperation = lastCompositeOperation; ctx.font = 'bold 70px sans-serif'; ctx.textAlign = 'center'; ctx.fillStyle = 'white'; ctx.strokeStyle = 'rgba(0,0,0,0.7)'; ctx.lineWidth = 8; ctx.strokeText('NEXT STAGE', width / 2, height / 2); ctx.fillText('NEXT STAGE', width / 2, height / 2); }
    ...
    let lastTime = 0;
    function animate(time) { if(!lastTime) lastTime = time; updatePlayer(time); updateCoins(); updateAttackEvents(time); updateProjectiles(); camera.x = player.worldX - (viewWidth / 2); camera.y = player.worldY - (viewHeight / 2); ctx.save(); ctx.scale(1 / CAMERA_ZOOM, 1 / CAMERA_ZOOM); renderWorld(time); drawPortal(time); drawCoins(time);  drawProjectiles(time); drawPlayer(time); ctx.restore();  if (!gameCleared) { drawControlButtons(); drawStageUI(); drawResetButton(); } if (gameCleared) { updateAndDrawClearEffects(); } requestAnimationFrame(animate); }
    ...
    function init(stageLevel = 1, isFullReset = false) { currentStage = stageLevel; width = canvas.width = window.innerWidth; height = canvas.height = window.innerHeight; viewWidth = width * CAMERA_ZOOM; viewHeight = height * CAMERA_ZOOM; updateControlButtonsPosition(); gameCleared = false; fireworksLaunched = false; rockets = []; particles = []; highestX = 0; hostileProjectiles = []; if (isFullReset) { currentMapSeed = Date.now() + Math.random(); iceCoins = []; rainbowCoins = []; redCoins = []; gambleCoins = []; attackEvents.forEach(e => soundManager.stopLoop(e.loopSound)); attackEvents = []; if(audioCtx && audioCtx.state === 'running') soundManager.playMusic('bgm'); } resetPlayer(); if (!spawnCheckTimer) { spawnCheckTimer = setInterval(spawnManager, SPAWN_CHECK_INTERVAL); } const startPlatformY = viewHeight - 100; const platforms = []; let currentX = -200; let prevY = startPlatformY; const startPlatformSegmentWidth = 100; const startPlatformSegmentHeight = startPlatformSegmentWidth / 1.7; for (let i = 0; i < 10; i++) { platforms.push({ worldX: currentX, worldY: prevY, width: startPlatformSegmentWidth, height: startPlatformSegmentHeight, isPhysical: true, initialY: prevY }); currentX += startPlatformSegmentWidth; } player.initialX = 150; player.initialY = startPlatformY - 150; player.worldX = player.initialX; player.worldY = player.initialY; const seededRandom = createSeededRandom(currentMapSeed); const s = stageLevel - 1; const platformCount = 10 + s * 5; const MIN_X_GAP_BASE = 110 + s * 15; const MAX_X_GAP_BASE = 160 + s * 20; const MAX_Y_CHANGE = 40 + s * 15; const platformMaxWidth = Math.max(60, 200 - s * 15); const platformMinWidth = Math.max(40, 100 - s * 10); let previousPlatformWasRainbow = false; let makeNextPlatformWider = false; for (let i = 0; i < platformCount; i++) { let MIN_X_GAP = MIN_X_GAP_BASE; let MAX_X_GAP = MAX_X_GAP_BASE; if (previousPlatformWasRainbow) { MIN_X_GAP *= 1.6; MAX_X_GAP *= 1.8; previousPlatformWasRainbow = false; } const xGap = MIN_X_GAP + seededRandom() * (MAX_X_GAP - MIN_X_GAP); const yChange = (seededRandom() - 0.45) * 2 * MAX_Y_CHANGE; let pW = platformMinWidth + seededRandom() * (platformMaxWidth - platformMinWidth); if (makeNextPlatformWider) { pW = Math.min(pW * 1.5, platformMaxWidth * 1.2); makeNextPlatformWider = false; } let pH = pW / 1.7; currentX += xGap; let newY = prevY + yChange; if (newY > viewHeight - pH - 20) newY = viewHeight - pH - 20; if (newY < 150) newY = 150; const newPlatform = { worldX: currentX, worldY: newY, width: pW, height: pH, isPhysical: true, initialY: newY }; if (i > 0 && !previousPlatformWasRainbow && seededRandom() < RAINBOW_PLATFORM_CHANCE) { newPlatform.type = 'rainbow'; previousPlatformWasRainbow = true; makeNextPlatformWider = true; } platforms.push(newPlatform); prevY = newY; } const portalX = currentX + MAX_X_GAP_BASE + 100; const portalHeight = 300; const portalWidth = 120; portal = { worldX: portalX, worldY: prevY - portalHeight / 2, width: portalWidth, height: portalHeight, isPhysical: false }; worldObjects = [ { worldX: -100000, worldY: -10000, width: 200000, height: 20000, isPhysical: false }, ...platforms ]; createPortalAssets(); }
