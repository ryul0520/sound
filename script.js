window.onload = function() {
    ...
    const BASE_GRAVITY = 1.0, BASE_JUMP_FORCE = -18, BASE_PLAYER_ACCEL = 1.8, BASE_FRICTION = 0.90, BASE_MAX_SPEED = 8;
    ...
    let gameStarted = false; // ### 추가: 게임 시작 상태 플래그

    // (이하 함수들은 변경이 거의 없으므로 생략하고, 변경된 핵심 함수들만 다시 작성합니다)
    // ... getStaticNoiseValue, createSeededRandom, createPlayerTexture, ... etc. ...
    ...
    window.addEventListener('click', (e) => { if (Math.sqrt((e.clientX - resetButton.x)**2 + (e.clientY - resetButton.y)**2) < resetButton.radius && !gameCleared) resetGame(); });
    ...
    function handleJumpInput() { if (!gameStarted) return; jumpBufferTime = Date.now(); }
    ...
    function updatePlayer(time) {
        ...
        PLAYER_ACCEL = BASE_PLAYER_ACCEL; MAX_SPEED = BASE_MAX_SPEED;
        ...
        if (jumpBufferValid && coyoteTimeValid) {
            ...
            if (isOnRainbow) { soundManager.playSound('super_jump'); player.dy = BASE_JUMP_FORCE * 1.8; } 
    ...
    function animate(time) {
        ...
        renderWorld(time);
        drawPortal(time); drawCoins(time); drawProjectiles(time); drawPlayer(time);
        ctx.restore(); 
    ...
    // ### 수정된 부분: 부활 버그 해결을 위한 init 함수 로직 순서 변경 ###
    function init(stageLevel = 1, isFullReset = false) {
        ...
        for (let i = 0; i < 10; i++) { 
            platforms.push({ worldX: currentX, worldY: prevY, width: startPlatformSegmentWidth, height: startPlatformSegmentWidth / 1.7, isPhysical: true });
        ...
        // 플레이어 시작 위치를 설정하고, 그 다음에 리셋 함수를 호출
        player.initialX = 150;
        player.initialY = startPlatformY - 150;
        resetPlayer(); // 모든 지형 생성이 끝난 후 플레이어 위치 리셋
    }
    
    const soundFiles = { jump: 'jump.mp3', super_jump: 'super_jump.mp3', hit: 'hit.mp3', boost: 'boost.mp3', freeze: 'freeze.mp3', alert: 'alert.mp3', danger_loop: 'danger_loop.mp3', gamble: 'gamble.mp3', invert: 'invert.mp3', clear: 'clear.mp3', bgm: 'bgm.mp3', firework: 'firework.mp3' };
    ...
    async function preloader() {
        ...
        window.addEventListener('resize', () => { if (gameStarted) init(currentStage, false); });
    }
    ...
    function startGame() {
        ...
        gameStarted = true;
        initAudioContext(); // 사용자의 첫 터치로 오디오 활성화
        
        // 애니메이션 루프에 게임 로직이 포함되므로, 플래그만 바꿔주면 자연스럽게 시작됨
        init(loadProgress(), true); 
        requestAnimationFrame(animate); // animate를 여기서 처음 호출
    }

    // 전역 이벤트 리스너: 화면 아무곳이나 클릭/터치하면 게임 시작
    window.addEventListener('click', startGame, { once: true });
    window.addEventListener('touchstart', startGame, { once: true });

    preloader();
    // 시작 전에 안내 문구를 표시하기 위해 animate를 미리 한번 호출
    requestAnimationFrame(animate);
};